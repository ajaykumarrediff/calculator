...
export default function HCF() {
  ...
  const [lcmValue, setLCMValue] = React.useState();
  ...
  const calculateLCM = () => {
    if (age == 2) {
      ...
      if (
        ...
        let lcm = calculateTLCM(a, calculateTLCM(b, c));
        setLCMValue(lcm);
      } else {
        setOpen(true);
      }
      ...
      if (
        ...
        let lcm = calculateTLCM(a, calculateTLCM(b, calculateTLCM(c, d)));
        setLCMValue(lcm);
      } else {
        setOpen(true);
      }
      ...
      if (
        ...
        let lcm = calculateTLCM(
          a,
          calculateTLCM(b, calculateTLCM(c, calculateTLCM(d, e)))
        );
        setLCMValue(lcm);
      } else {
        setOpen(true);
